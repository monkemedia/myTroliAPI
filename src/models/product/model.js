...
productSchema.statics.findAllProducts = async ({ page, limit }) => {
  const products = await Product.find({}).sort('-created_at').populate('variants variant_options categories').skip((page - 1) * limit).limit(limit)
...
productSchema.statics.findById = async (_id) => {
  const product = await Product.findOne({ _id }).populate('variants variant_options categories')
  return product
}
